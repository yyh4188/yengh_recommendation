...
class Actor(nn.Module):
    ...
    def __init__(self, input_dim, action_dim, hidden_size, init_w=2e-1, temperature=0.1):
    ...
    def contrastive_loss(self, actions, labels):
        """
        计算对比学习损失
        :param actions: 批量动作向量 [batch_size, action_dim]
        :param labels: 标签，表示哪些样本属于同一用户 [batch_size]
        :return: 对比学习损失
        """
        # 归一化动作向量
        ...
        # 计算相似度矩阵
        similarity = torch.matmul(actions, actions.T) / self.temperature
        ...
        # 只计算正样本的损失
        positive_sum = (mask * log_prob).sum(dim=1)
        mask_sum = mask.sum(dim=1)
        
        # 安全检查，避免除以零
        valid_mask = mask_sum > 0
        if valid_mask.sum() > 0:
            loss = - positive_sum[valid_mask] / mask_sum[valid_mask]
            loss = loss.mean()
        else:
            # 如果没有正样本，返回0损失
...
class AttentionMechanism(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim):
        ...
        self.attention = nn.Sequential(
            nn.Linear(embedding_dim + 1, hidden_dim),  # 输入：嵌入向量 + 评分
            nn.ReLU(),
            nn.Linear(hidden_dim, 1)
        )
    
    def forward(self, items_emb, ratings):
        """
        计算注意力权重
        :param items_emb: 物品嵌入向量 [batch_size, seq_len, embedding_dim]
        :param ratings: 评分向量 [batch_size, seq_len, 1]
        :return: 加权后的嵌入向量 [batch_size, embedding_dim]
        """
        # 拼接嵌入向量和评分
        inputs = torch.cat([items_emb, ratings], dim=2)
        ...
        # 加权求和
        weighted_emb = torch.sum(attention_weights * items_emb, dim=1)
        
        return weighted_emb, attention_weights
